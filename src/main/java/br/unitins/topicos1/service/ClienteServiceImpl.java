...
public class ClienteServiceImpl implements ClienteService {
    ...
    public ClienteResponseDTO update(ClienteDTO dto, Long id) {
        ...
        if (dto.senha() != null) cliente.setSenha(dto.senha());
        if (dto.email() != null) cliente.setEmail(dto.email());
        if (dto.cpf() != null) cliente.setCpf(dto.cpf());
        if (dto.telefones() != null) cliente.setTelefone(dto.telefones().stream().map(TelefoneDTO::valueOf).toList());
        if (dto.enderecos() != null) cliente.setEndereco(dto.enderecos().stream().map(EnderecoDTO::valueOf).toList());
        if (dto.dataNascimento() != null) cliente.setDataNascimento(dto.dataNascimento());
        if (dto.login() != null) cliente.setLogin(dto.login());
        if (dto.sobrenome() != null) cliente.setSobrenome(dto.sobrenome());
 ...
 @Override
    ...
    public Boolean alterarSobrenome(AlterarSobrenomeDTO dto, Long id) {
        Cliente cliente = repository.findById(id);
        if(cliente != null){
               cliente.setSobrenome(dto.sobrenomeNovo());
                repository.persist(cliente);
        }
        return false;
    }
